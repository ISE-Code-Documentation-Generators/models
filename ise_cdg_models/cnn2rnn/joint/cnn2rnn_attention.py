...
class CNN2RNNAttention(nn.Module):
    def __init__(
        self,
        src_vocab,
        md_vocab,
        ...
        self.decoder = DocumentDecoderAttention(
            md_vocab_helper.get_embedding(md_embed_size, decoder_vectype), 
            md_vocab_helper.vocab_size, md_embed_size, 
            self.encoder.source_image_cnn.context_hidden_size, 
            self.encoder.source_image_cnn.output_hidden_size,
        )
    ...
    def forward(self, source, markdown, device,
        ...
        output_sequence, code_image_context = self.encoder(source)
        hidden = code_image_context

        x = markdown[0] # shape: (batch)
        
        for t in range(1, target_sequence_len):
            output, hidden = self.decoder(x, hidden, output_sequence)
            outputs[t] = output
